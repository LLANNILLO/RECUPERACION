window.addEventListener("load", () => {
  ...
  eliminarDeListaBtn.addEventListener("click", () => {
    let lista = document.querySelector("ul");
    let elementosLista = lista.children;

    let posicion = prompt(
      `Eliminar elementos del 1 al ${elementosLista.length}`
    );

    if (!isNaN(posicion) && 1 < posicion < elementosLista.length - 1) {
      lista.removeChild(elementosLista[posicion]);
    }
  });
});
